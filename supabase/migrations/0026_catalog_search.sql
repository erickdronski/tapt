-- Public, paginated search over the whole beer catalog (name + brewery fuzzy,
-- optional style + NA/low filters). Backs the in-app Catalog browse screen.
...
as $$
  ...
  select id, name, style, abv, is_na_low, brewery_name, country, image_url,
         count(*) over() as total
  from base
  order by (image_url is null), lower(name)
