-- Audit fix (privacy/PII): public_profile() leaked identifying fields for
-- PRIVATE and BLOCKED profiles. The early-return branch still emitted
-- display_name, handle, avatar_url, region and member_since; since social_visible
-- defaults to false for every user, that branch is the normal path for a private
-- account, so anyone could pull a private user's face photo, @handle, real name,
-- region and join date by passing their id. Now the branch returns NO identifying
-- fields -- only the visibility envelope. The app renders a neutral placeholder.
