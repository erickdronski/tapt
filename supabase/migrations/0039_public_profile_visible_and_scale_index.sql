-- 0039_public_profile_visible_and_scale_index.sql
--
-- Two corrections found while verifying the follow graph for global scale:
--  1) public_profile counted check-ins with a blocklist (not in removed/rejected/
--     hidden). The real moderation convention is visible/hidden/removed/under_review,
--     so an 'under_review' pour leaked into passport stats while the friends feed
--     (correctly) only shows 'visible'. Align to `moderation_status = 'visible'`
--     everywhere so the card shows exactly what is publicly visible.
--  2) The followers count scans follow by followee_id, which had no index (only the
--     (follower_id, followee_id) pkey covers the follower direction). Add it so
--     follower counts stay fast as the graph grows globally.
-- Verified live: self card still returns pours/country/top-style/favorite correctly.

create index if not exists follow_followee_idx on public.follow (followee_id);
